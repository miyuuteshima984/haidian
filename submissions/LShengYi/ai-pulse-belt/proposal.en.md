...
summary: "AI Pulse Belt: the engineering tradition of the centennial Jing-Zhang Railway 'iron pulse' — inspectable, verifiable, retired — is translated into a public protocol for the AI-era 'digital pulse belt': before any AI service enters public space it must be declarable, testable, releasable, and retirable (P1-P4); the tabletop simulation of 15 services concludes that none can be released directly; the flat-line archive wall publicly exhibits every retired service as governance evidence. The three-level scope, the three zones & two wings, and 59 recomputable metrics (44 known) fully respond to announcement clauses 1.4/1.5."
...
| Implementation feasibility | All 12 renewal projects hang on P1-P4 protocol beats; no project enters the next beat without passing evidence; the 15-service tabletop simulation gives reproducible per-beat conclusions and default failure actions | Ch. 11 (renewal/phasing/funding) | risk.json, simulation.json, phasing.geojson | [depth:renewal_project_list] [depth:phasing_implementation] |
| AI planning innovation | The Pulse Protocol is not a flowchart but urban infrastructure: each of the four beats has a spatial interface (P1 declaration desk / P2 test sandbox / P3 status light / P4 flat-line archive wall), binding AI capability to space, transport, public services, culture, and governance item by item | Ch. 6 (scenarios/Pulse Protocol) | simulation.json (15-service four-beat simulation) | [source:AGENT-TASKBOOK] |
| Expression completeness | 17 chapters bilingual 1:1, 59 metrics with 44 known and recomputable, 9 geometry layers topologically validated, figures/PDF/HTML fully aligned | Full text + drawings/HTML/visuals | manifest.json, assets/figures/*, drawings/* | [metric:site_area_sqm] [metric:green_ratio] |
...
**Table A3 Clause-by-clause response to mandatory items**

| Announcement clause | Mandatory item | Response location |
| --- | --- | --- |
| 1.3 Three solicitation purposes | World-class AI ecosystem, new urban form, talent-desired district | Ch. 3, Ch. 6 |
| 1.4 Three-level scope | Research/overall-design/key areas | Ch. 3 |
| 1.5(1) Three zones & two wings | Coordination loop of the three key areas and two wings | Ch. 3, Ch. 5 |
| 1.5(2)1 Industry goals & functional layout | "1+X+1" functional proportions and spatial organization | Ch. 3 (Table 1) |
| 1.5(2)2 Urban renewal master framework | Renewal project list and phased implementation | Ch. 11 |
| 1.5(2)3 Transport/rail/utilities | Station integration, road micro-circulation, slow traffic, parking | Ch. 9 |
| 1.5(2)4 Jing-Zhang Ruins Park vitality belt | Greenway connection, heritage activation, park coordination | Ch. 10 |
| 1.5(2)5 Urban character | BFA and other art resources, form control | Ch. 10 |
| 1.5(3)1 Zhongzhiyuan | National AI-platform opportunity, agglomeration positioning, external transport upgrade | Ch. 5 |
| 1.5(3)2 Origin Community | Results translation, talent community, external transport links | Ch. 5 |
| 1.5(3)3 Dazhongsi | Intelligent economy block, station-city integration, retain/renovate/demolish | Ch. 5 |
...
| --- | --- | --- |
| known, directly recomputable | 44 | Spatial class 14 (areas/ratios/counts, EPSG:4548) + functional proportions 7 (concept-layer recalculation) + element counts 14 (registered in narrative) + mechanism coverage 9 (8 coverage ratios verified =1.0, plus an 8-dimension risk-registry count); full list in the Metrics chapter |
...
- **Generator—artifact pipeline**: geometric layers (9 GeoJSON files), the metric system, figures (PNG), the booklet/boards (PDF), and the interactive page (HTML) are all produced deterministically by the `tools/gen_0*.py` generators from the single input `geometry/provisional_boundaries.geojson` (fixed random seed, byte-level manifest-hash reproduction, verified at gate G1).
...
| --- | --- | --- |
| Railway and heritage sites | Jing-Zhang railway historic alignment, Qinghuayuan Station site, Jing-Zhang heritage park vitality belt (announcement 1.5(2)4) | Precise current track alignment awaits official drawings; expressed conceptually [data:geometry/constraints.geojson#CONSTRAINTS] |
...
**Executable protocol registry and tabletop conclusion**: the machine-readable tabletop record of the protocol is `simulation.json` — 15 public AI services (12 scenario cards + 3 test scenarios) each checked against the five P1 declaration elements, P2 testing status, the P3 wayfinding status light, and the five rollback trigger classes, verifiable by a tabletop script [data:simulation.json] [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK]. **Tabletop conclusion: 15/15 services pass the five-element P1 declaration check; 5 have completed the P2 test simulation and 10 remain untested; 0 services have been released under P3** — no service can enter public space today, and all must first pass controlled testing and objection release. This is the mirror image of "no service may linger indefinitely in pilot, and none may be released without testing": the protocol does not endorse the proposal's scenarios, it sets the admission conditions for each of them. The record is concept expression and does not mean any service is implemented or approved.
...
| JZ-09 | Low-altitude delivery route verification | New infra/industry test | Airspace and safety-supervision review | Airspace approval | ≥90 test days, zero tolerance for accidents | District + regulator [data:geometry/constraints.geojson#CONSTRAINTS] | P2 test |
...
| JZ-11 | Barrier-free AI wayfinding system | Public service/accessibility | Standards and data-authorization review | Accessibility-standard re-check | ≥30 coverage points, 100% manual backup | Disabled federation + operator [data:geometry/constraints.geojson#CONSTRAINTS] | P1 declare |
...
| Timeline | 0–3 months project initiation (P1 declare) → 3–6 months temporary stitching and deployment (P2 test) → 6–9 months monitoring and evaluation → 9–12 months transition to formal or retirement (P3/P4) |
...
| Days 15–30 | Close review feedback: revise chapters and metrics per comments | Proposal team | Revision (v9.x) |
...
**Table B2 Quality gates G0–G4**

| Gate | Content | Verification | Current result |
| --- | --- | --- | --- |
| G0 Content self-check | Resolvable citations, numeric consistency, closed ranges, bilingual 1:1 | Scripted checks + Tables A1–A7 above | PASS |
| G1 Deterministic validation | Manifest hash reproduction, byte-level recomputability | finalize_submission + self_check DETERMINISTIC_VALIDATION | PASS |
| G2 Spatial review | Topology/CRS/seamless coverage/boundary-deviation disclosure | self_check SPATIAL_REVIEW | PASS |
| G3 Visual packaging | Figure size/PDF page count/offline visualization zero external links | self_check VISUAL_PACKAGING | PASS |
| G4 Release review | Review-dimension evidence index, deviation statement, feedback closure | Tables A1–A7 published with the review | Updated with review |
...
The indicator system (`metrics.json`) has **59 metrics** in six classes (all 59 are listed with values in the table below):
...
| P1 declaration pass rate | 15/15 = 1.0 | High (simulation.json verification) | All 15 services pass five-element P1 check |
...
| Investment magnitudes | 11 one-off items + 1 annual operation, item ranges | Ch. 11 investment-magnitude matrix |
...
| Function match | Chapter 2 three-positioning-five-function mapping table; compliance_matrix.json 1.3/1.5 | Three positionings, five functions, Three Zones and Two Wings layout, and "1+X+1" industrial system each have a mapping table checked item by item | Closed | Item-by-item check against official industry catalog |
| Brand recognition | Chapter 2 naming, Chapter 9 VI and brand-extension 3 tables | AI Pulse Belt naming system, logo semiotics, differentiation comparison and derived-application argument | Pending | VI elements require official approval for implementation |
