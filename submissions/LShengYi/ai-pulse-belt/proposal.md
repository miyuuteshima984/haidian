...
summary: "智脉一带 · AI Pulse Belt：京张铁路百年'铁脉'的工程传统——可巡检、可验收、可退役——被转译为 AI 时代'数字智脉'的公共协议：任何 AI 服务进入公共空间之前，都必须可申报、可测试、可发布、可退役（P1-P4）；15 项服务桌面推演结论为零项可直接发布；平线档案墙把每一项退役服务公开陈列为治理证据。三层范围、三区两翼与 59 项可复算指标（44 known）完整回应公告 1.4/1.5。"
...
| P3 它凭什么留下？ | 导视状态灯可视化（稳定波形=正常运行、脉冲闪烁=测试中、平线=已停用）与底线指标公开 | 导视状态灯节点，全带可读 [data:geometry/constraints.geojson#CONSTRAINTS] | 三道异议门无异议、五项底线指标达标 | 降级回 P2；运行边界失效即停止服务并恢复场地 |
...
四问的判定证据全部登记于 `simulation.json`（15 项服务逐项核对）与第六章协议表，评审可按行复核 [metric:pulse_beat_count] [metric:rollback_trigger_class_count] [metric:objection_gate_count]。四问之外，空间本身给出双重证明：**三层范围**（43.6 km²/11.413 km²/368.4 ha）与"一带三核双翼多点"结构回应公告 1.4/1.5（偏差见表 A6）；**平线档案墙**让"退役"成为可审计的公共资源而非失败掩盖——这是四问中 P4 的空间化表达。全部内容为概念建议，官方边界、控规与现状调查发布后按 P4 程序重算。
...
| 可实施性 | 12 项更新项目全部挂接 P1-P4 协议节拍，放行证据未过不进入下一节拍；15 项服务桌面推演给出可复现的分拍结论与失败默认动作 | 第十章（更新/分期/资金） | risk.json、simulation.json、phasing.geojson | [depth:renewal_project_list] [depth:phasing_implementation] |
| AI 规划创新 | 脉冲协议不是流程图而是城市基础设施：四拍各配空间界面（P1 申报台/P2 测试沙盒/P3 状态灯/P4 平线档案墙），AI 能力与空间、交通、公共服务、文化和治理逐项结合 | 第六章（场景/脉冲协议） | simulation.json（15 项服务四拍推演） | [source:AGENT-TASKBOOK] |
| 表达完整度 | 15 章双语 1:1、59 项指标 44 known 可复算、9 类图层拓扑校验、图件/PDF/HTML 全对齐 | 全文＋图纸/HTML/视觉 | manifest.json、assets/figures/*、drawings/* | [metric:site_area_sqm] [metric:green_ratio] |
| 原创性 | "铁脉→智脉"的工程传统转译 + 平线档案墙（退役即公开证据）+ 波形状态灯语言：原创来自机制而非辞藻 | 执行摘要＋第八章 | 命名体系分级表、波形状态灯语言 | [source:JZ-RAILWAY-CULTURE] |
...
| agent 任务 | 响应章节 | 主要交付 |
| --- | --- | --- |
| agent.1 命名体系 | 执行摘要、第八章 | 智脉一带命名体系分级表、Logo 方向、口号 |
...
| agent.5 文化叙事 | 第八章 | 三线文化叙事、波形状态灯导视系统 |
...
| 公告条款 | 必答要求 | 回应位置 |
| --- | --- | --- |
| 1.3 三大征集目的 | 世界级 AI 创新生态、新型城市形态、人才向往城区 | 第三章、第六章 |
| 1.4 三层范围 | 统筹研究区/总体设计区/三重点区 | 第二章 |
| 1.5(1) 三区两翼 | 三重点区与双翼协同回路 | 第三章、第五章 |
| 1.5(2)1 产业目标与功能布局 | "1+X+1"功能比例与空间组织模式 | 第三章（本章功能映射表） |
| 1.5(2)2 城市更新总体框架 | 更新项目清单与分期实施 | 第十章 |
| 1.5(2)3 交通轨道市政配套 | 站点一体化、道路微循环、慢行、停车 | 第九章 |
| 1.5(2)4 京张遗址公园活力带 | 绿廊贯通、遗址活化、公园统筹 | 第八章 |
| 1.5(2)5 城市风貌 | 北影等艺术资源、风貌控制 | 第八章 |
| 1.5(3)1 众智园 | 国家人工智能平台契机、集聚区定位、对外交通提升 | 第五章 |
| 1.5(3)2 原点社区 | 成果转化、人才社区、对外交通联系 | 第五章 |
| 1.5(3)3 大钟寺 | 智能经济街区、站城一体、拆改留 | 第五章 |
...
| 口径 | 数量 | 说明 |
| --- | --- | --- |
| known 可直接复算 | 44 项 | 空间类 14（面积/比例/计数，EPSG:4548 复算）＋功能比例 7（概念图层复算）＋元素计数 14（正文登记）＋机制 coverage 9（8 项覆盖率核验式全部=1.0，另风险登记 8 维计数），清单见指标章 |
...
- **生成器—产物管线**：几何图层（9 个 GeoJSON）、指标体系、图件（PNG）、书册/展板（PDF）、交互页（HTML）均由 `tools/gen_0*.py` 生成器从单一输入 `geometry/provisional_boundaries.geojson` 确定性产出（固定随机种子、字节级 manifest 哈希复现，经 G1 门校验）。
...
| 类型 | 官方表述 | 本方案对应载体 |
| --- | --- | --- |
| 三大定位 1 | 百年京张文化带 | 第八章三线文化叙事、智脉艺术铁轨与平线档案墙 |
...
**同任务等价（概念建议，回应公共包容维度）**：与「服务合同」配套的另一项验收规则是——**拒绝使用 AI、不同意非必要数据授权或没有智能设备的市民，仍能通过人工/低技术路径完成同一项基本任务**；若人工路径缺失、额外收费或长期不可用，对应 AI 服务不得继续开放。12 张场景卡服务合同表中的「人工复核」列即该规则的落实载体，无障碍 AI 导行站另以 100% 人工替代兜底（卡 08）；全部替代通道与费用豁免登记于 P4 复盘公开 [metric:scenario_card_count]。
...
**协议可执行登记与推演结论**：协议的机器可读推演登记见 `simulation.json`——15 项公共 AI 服务（12 张场景卡 + 3 个测试场景）逐项核对 P1 申报五要素、P2 测试状态、P3 导视状态灯与五类回滚触发器，可由表推演脚本复核 [data:simulation.json] [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK]。**推演结论：15/15 项服务通过 P1 申报五要素核验；5 项完成 P2 测试推演、10 项待测；0 项获得 P3 发布放行**——没有任何一项服务可以在今天直接进入公共空间运行，全部须先通过受控测试与异议放行。这与"任何服务不能无限期停留在试点状态，也不能未经测试直接发布"互为表里：协议不是给方案背书，而是给每一项服务设定准入条件。登记为概念表达，不代表任何服务已实施或已获批。
...
| JZ-09 | 低空配送航线验证场 | 新基建/产业测试 | 空域与安全监管梳理 | 空域审批 | 测试≥90 天、事故零容忍 | 属地+监管机构 [data:geometry/constraints.geojson#CONSTRAINTS] | P2 测试 |
| JZ-10 | 端侧算力与能耗调控示范楼 | 新基建/市政 | 能源与算力需求评估 | 消防与运营主体确认 | 能耗下降≥15%、PUE 可控 | 能源企业+园区 [data:geometry/buildings.geojson#BLDG-001] | P2 测试 |
| JZ-11 | 无障碍 AI 导行系统 | 公共服务/无障碍 | 标准与数据授权梳理 | 无障碍设施标准复核 | 覆盖率≥30 项、人工替代 100% | 残联+运营方 [data:geometry/constraints.geojson#CONSTRAINTS] | P1 申报 |
...
| 时间线 | 0–3 月立项（P1 申报）→ 3–6 月临时缝合与部署（P2 测试）→ 6–9 月监测评估 → 9–12 月转正式或退役（P3/P4） |
...
| 第 15–30 天 | 评审反馈闭环：按意见修订章节与指标 | 本方案团队 | 修订版（v9.x） |
...
| 质量门 | 内容 | 校验方式 | 当前结果 |
| --- | --- | --- | --- |
| G0 内容自检 | 引用可解析、数字自洽、区间封闭、双语 1:1 | 脚本化检查＋本表 A1–A7 核对 | PASS |
| G1 确定性校验 | manifest 哈希复现、字节级可复算 | finalize_submission + self_check DETERMINISTIC_VALIDATION | PASS |
| G2 空间审查 | 拓扑/CRS/无缝覆盖/边界偏差披露 | self_check SPATIAL_REVIEW | PASS |
| G3 视觉打包 | 图尺寸/PDF 页数/离线可视化零外链 | self_check VISUAL_PACKAGING | PASS |
| G4 发布评审 | 评审维度证据索引、差异声明、反馈闭环 | 本表 A1–A7 随评审公开 | 随评审更新 |
...
指标体系（`metrics.json`）含 **59 项**，分六类管理（表内列出全部 59 项及数值）：
...
| P1 申报通过率 | 15/15 = 1.0 | 高（simulation.json 核验） | 15 项服务全部通过 P1 五要素核验 |
...
| 投入量级 | 11 项一次性建设＋1 项年度运营，逐项区间 | 第十一章投入量级矩阵 |
